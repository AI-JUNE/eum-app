/**
 * 이음 시드 데이터 — 2R 가이드 §6-4 「하드코딩 페르소나 → 시드 스크립트 이관」
 *
 * 화면 코드가 아니라 여기가 시드의 단일 출처다. 모든 레코드에 `is_demo: true` 가 붙는다.
 *   - 실데이터(실제 가입 계정)와 같은 테이블에 섞여도 플래그로 구분된다(§6-4).
 *   - 통계·실적 집계에서 제외하려면 `excludeDemo(rows)` 를 쓴다.
 *   - 화면에는 `SEED_META.badge` (「데모 데이터」) 배지를 단다(§1 원칙 4, §7-4).
 *
 * 이 모듈은 React·테마에 의존하지 않는다. 아바타 색은 팔레트를 주입받는다.
 * 그대로 Supabase 시드 스크립트(seed.sql 생성기)로 재사용할 수 있다.
 */

/** 시드 레코드 판별 플래그 이름 — DB 컬럼명과 동일하게 유지 */
export const DEMO_FLAG = 'is_demo';

export const SEED_META = Object.freeze({
  version: 'seed-2R-v1',
  badge: '데모 데이터',
  notice: '아래 정보는 시연용 데모 데이터입니다. 실제 이용자 정보가 아닙니다.',
  source: 'packages/db/seed.js',
  region: '광주광역시 광산구 우산동',
});

/** 기본 팔레트 — 호출부가 디자인 토큰을 주입하지 않을 때의 대체값 */
export const DEFAULT_PALETTE = Object.freeze({
  sage: '#7C9A83', lavender: '#8F86B8', peach: '#E0A183',
});

/** 레코드 1건이 시드(데모)인지 */
export function isDemoRecord(row) {
  return !!(row && row[DEMO_FLAG] === true);
}

/** 통계·실적 집계에서 시드를 제외 (§6-4 「통계에서 제외 가능」) */
export function excludeDemo(rows) {
  return (rows || []).filter((r) => !isDemoRecord(r));
}

/** 실데이터만 골라내기의 반대 — 시드만 */
export function onlyDemo(rows) {
  return (rows || []).filter(isDemoRecord);
}

/** 객체 트리의 모든 레코드 배열에 is_demo 를 심는다(원본 불변). */
function markDemo(data) {
  const out = {};
  for (const [table, rows] of Object.entries(data)) {
    out[table] = Array.isArray(rows)
      ? rows.map((r) => (r && typeof r === 'object' && !Array.isArray(r) ? { ...r, [DEMO_FLAG]: true } : r))
      : rows;
  }
  return out;
}
...
/**
 * 시드 데이터 생성. 값·구조는 이관 전 SEED_DATA 와 100% 동일하고
 * 각 레코드에 is_demo:true 만 추가된다.
 * @param {{sage?:string, lavender?:string, peach?:string}} palette 아바타 색 토큰
 */
export function buildSeed(palette) {
  const P = { ...DEFAULT_PALETTE, ...(palette || {}) };
  const data = {
    participants: [
      // 청년 5명
      { id: 'p001', name: '김민준', gender: 'M', type: 'youth', age: 27, phone: '010-1234-5678', address: '광주광역시 광산구 우산동', emergency_contact: '010-9876-5432 (부친)', occupation: '스타트업 개발자', skills: ['디지털코칭', '학습멘토', '코딩교육'], interests: ['IT', '진로상담', '여행'], availability: ['평일저녁', '토요일'], status: 'active', avatar_color: P.sage, joined_at: '2027-03-15', bio: '마곡 스타트업 2년차 개발자. 어르신께 IT를, 아이들에게 코딩을 가르쳐드리고 싶어요.' },
      { id: 'p002', name: '이지원', gender: 'F', type: 'youth', age: 25, phone: '010-2345-6789', address: '광주광역시 광산구 우산동', emergency_contact: '010-1111-2222 (모친)', occupation: '대학원생', skills: ['학습멘토', '글쓰기', '독서지도'], interests: ['교육', '문학', '심리'], availability: ['평일저녁', '주말'], status: 'active', avatar_color: P.sage, joined_at: '2027-03-18', bio: '교육학 석사과정. 아이들과 책 읽고 글쓰기를 함께하고 싶어요.' },
      { id: 'p003', name: '박서준', gender: 'M', type: 'youth', age: 29, phone: '010-3456-7890', address: '광주광역시 광산구 첨단동', emergency_contact: '010-3333-4444 (형)', occupation: '디자이너', skills: ['디지털코칭', '예술교육', '사진'], interests: ['디자인', '사진', '카페'], availability: ['토요일', '일요일'], status: 'active', avatar_color: P.sage, joined_at: '2027-03-20', bio: 'UX 디자이너. 어르신께 스마트폰 사진을, 아이들에게 그림을 가르쳐요.' },
      { id: 'p004', name: '최예린', gender: 'F', type: 'youth', age: 26, phone: '010-4567-8901', address: '광주광역시 광산구 우산동', emergency_contact: '010-5555-6666 (모친)', occupation: '간호사', skills: ['건강관리', '응급처치', '돌봄'], interests: ['건강', '운동', '요리'], availability: ['평일저녁'], status: 'active', avatar_color: P.sage, joined_at: '2027-04-01', bio: '대학병원 간호사. 어르신 건강 케어와 아이 안전에 강점이 있어요.' },
      { id: 'p005', name: '정태윤', gender: 'M', type: 'youth', age: 28, phone: '010-5678-9012', address: '광산구 등촌동', emergency_contact: '010-7777-8888 (모친)', occupation: '회계사', skills: ['학습멘토', '수학교육'], interests: ['경제', '독서', '러닝'], availability: ['평일저녁', '토요일'], status: 'verifying', avatar_color: P.sage, joined_at: '2027-05-12', bio: '회계사. 아이들에게 수학과 경제 개념을 쉽게 알려주고 싶어요.' },

      // 어르신 5명
      { id: 'p101', name: '박순자', gender: 'F', type: 'senior', age: 73, phone: '010-1111-1111', address: '광주광역시 광산구 우산동 (42년 거주)', emergency_contact: '010-2222-3333 (딸)', occupation: '前 초등학교 교사', skills: ['독서지도', '서예', '동화구연'], interests: ['손주', '드라마', '꽃'], needs: ['스마트폰 도움', '말벗'], availability: ['평일오전', '평일오후'], status: 'active', avatar_color: P.lavender, joined_at: '2027-03-16', bio: '40년 교직 생활. 손주 같은 아이에게 옛이야기 들려주고 싶어요.' },
      { id: 'p102', name: '김복례', gender: 'F', type: 'senior', age: 78, phone: '010-2222-2222', address: '광주광역시 광산구 우산동 (30년 거주)', emergency_contact: '010-4444-5555 (아들)', occupation: '前 봉제공장 운영', skills: ['바느질', '뜨개질', '요리'], interests: ['요리', '드라마', '산책'], needs: ['장보기', '말벗'], availability: ['평일오후'], status: 'active', avatar_color: P.lavender, joined_at: '2027-03-22', bio: '평생 봉제일. 아이들에게 손바느질을 가르쳐주고 싶어요.' },
      { id: 'p103', name: '이병호', gender: 'M', type: 'senior', age: 71, phone: '010-3333-3333', address: '광주광역시 광산구 우산동', emergency_contact: '010-6666-7777 (딸)', occupation: '前 공무원', skills: ['역사이야기', '바둑', '서예'], interests: ['역사', '바둑', '등산'], needs: ['산책', '말벗'], availability: ['평일오전', '토요일'], status: 'active', avatar_color: P.lavender, joined_at: '2027-03-25', bio: '공무원 40년 정년퇴직. 청년들에게 인생 조언을, 아이들에게 역사 이야기를 들려주고 싶어요.' },
      { id: 'p104', name: '정금자', gender: 'F', type: 'senior', age: 75, phone: '010-4444-4444', address: '광주광역시 광산구 우산동', emergency_contact: '010-8888-9999 (며느리)', occupation: '前 동네 식당 운영', skills: ['요리', '한식', '이야기'], interests: ['요리', '드라마', '꽃밭'], needs: ['병원 동행', '집안일'], availability: ['평일오전', '평일오후'], status: 'active', avatar_color: P.lavender, joined_at: '2027-03-28', bio: '평생 식당. 아이들에게 손맛 김치 담그기를 가르쳐주고 싶어요.' },
      { id: 'p105', name: '윤석철', gender: 'M', type: 'senior', age: 70, phone: '010-5555-5555', address: '광주광역시 광산구 우산동', emergency_contact: '010-0000-1111 (아들)', occupation: '前 자영업', skills: ['장기', '한자', '경험담'], interests: ['장기', '뉴스', '걷기'], needs: ['말벗', '산책'], availability: ['평일오전'], status: 'active', avatar_color: P.lavender, joined_at: '2027-04-05', bio: '동네 토박이. 청년에게 사업 경험을 나누고 아이와 장기 두고 싶어요.' },

      // 양육가정 3가구
      { id: 'p201', name: '이서영', gender: 'F', type: 'parent', age: 38, phone: '010-6666-7777', address: '광주광역시 광산구 우산동', emergency_contact: '010-1010-2020 (배우자)', occupation: 'IT기업 PM (마곡)', skills: [], interests: [], availability: ['평일 저녁 7시 이후 픽업 가능'], status: 'active', avatar_color: P.peach, joined_at: '2027-03-19', child_id: 'p301', bio: '맞벌이라 퇴근 후 아이 돌봄 공백이 늘 걱정이에요.' },
      { id: 'p202', name: '한지영', gender: 'F', type: 'parent', age: 35, phone: '010-7777-8888', address: '광주광역시 광산구 우산동', emergency_contact: '010-3030-4040 (시어머니)', occupation: '간호사', skills: [], interests: [], availability: ['교대근무'], status: 'active', avatar_color: P.peach, joined_at: '2027-03-26', child_id: 'p302', bio: '교대근무라 정해진 픽업 시간이 어려워요. 안전한 공간에서 다양한 어른과 만나길 바라요.' },
      { id: 'p203', name: '김혜진', gender: 'F', type: 'parent', age: 40, phone: '010-8888-9999', address: '광주광역시 광산구 우산동', emergency_contact: '010-5050-6060 (배우자)', occupation: '교사', skills: [], interests: [], availability: ['주중 하원 후 ~ 저녁 6시'], status: 'active', avatar_color: P.peach, joined_at: '2027-04-02', child_id: 'p303', bio: '아이가 외동이라 다양한 세대와의 교류가 절실해요.' },

      // 아동 3명
      { id: 'p301', name: '김유진', gender: 'F', type: 'child', age: 8, phone: '', address: '광주광역시 광산구 우산동', emergency_contact: '010-6666-7777 (모친 이서영)', occupation: '초2', skills: [], interests: ['그림', '책', '강아지'], availability: [], status: 'active', avatar_color: P.peach, joined_at: '2027-03-19', parent_id: 'p201', bio: '책 읽기를 좋아하고 그림 그리는 걸 즐겨요.' },
      { id: 'p302', name: '한도윤', gender: 'M', type: 'child', age: 9, phone: '', address: '광주광역시 광산구 우산동', emergency_contact: '010-7777-8888 (모친 한지영)', occupation: '초3', skills: [], interests: ['로봇', '레고', '축구'], availability: [], status: 'active', avatar_color: P.peach, joined_at: '2027-03-26', parent_id: 'p202', bio: '레고와 로봇을 좋아하고 축구를 잘해요.' },
      { id: 'p303', name: '김지안', gender: 'F', type: 'child', age: 7, phone: '', address: '광주광역시 광산구 우산동', emergency_contact: '010-8888-9999 (모친 김혜진)', occupation: '초1', skills: [], interests: ['공룡', '책', '노래'], availability: [], status: 'active', avatar_color: P.peach, joined_at: '2027-04-02', parent_id: 'p203', bio: '공룡에 푹 빠져 있고 노래 부르기를 좋아해요.' },

      { id: 'p006', name: '강하늘', gender: 'M', type: 'youth', age: 24, phone: '010-6161-6262', address: '광주광역시 광산구 우산동', emergency_contact: '010-6363-6464 (모친)', occupation: '체육지도사', skills: ['장기', '운동지도', '진로멘토'], interests: ['운동', '바둑', '봉사'], availability: ['평일오전', '토요일'], status: 'active', avatar_color: P.sage, joined_at: '2027-05-20', bio: '생활체육지도사. 어르신과 장기 두고 아이들과 몸으로 노는 걸 좋아해요.' },
      { id: 'p007', name: '문지호', gender: 'M', type: 'youth', age: 30, phone: '010-7171-7272', address: '광주광역시 광산구 우산동', emergency_contact: '010-7373-7474 (배우자)', occupation: '사회복지사', skills: ['상담', '학습멘토', '돌봄'], interests: ['복지', '독서', '커피'], availability: ['평일저녁', '주말'], status: 'active', avatar_color: P.sage, joined_at: '2027-06-10', bio: '복지관 근무 사회복지사. 세대를 잇는 일에 진심이에요. (매칭 대기 중)' },
      { id: 'p008', name: '배수진', gender: 'F', type: 'youth', age: 23, phone: '010-8181-8282', address: '광주광역시 광산구 우산동', emergency_contact: '010-8383-8484 (모친)', occupation: '대학생(사회복지학)', skills: ['학습멘토', '돌봄', '글쓰기'], interests: ['교육', '봉사', '글쓰기'], availability: ['평일오후', '주말'], status: 'pending', avatar_color: P.sage, joined_at: '2027-07-12', bio: '사회복지학과 4학년. 실습으로 시작했지만 진짜 이웃이 되고 싶어요. (서류 검토 중)' },
      { id: 'p107', name: '천만복', gender: 'M', type: 'senior', age: 74, phone: '010-9191-9292', address: '광주광역시 광산구 우산동 (28년 거주)', emergency_contact: '010-9393-9494 (아들)', occupation: '前 목공소 운영', skills: ['목공', '손재주', '경험담'], interests: ['목공', '뉴스', '산책'], needs: ['병원 동행', '말벗'], availability: ['평일오전'], status: 'pending', avatar_color: P.lavender, joined_at: '2027-07-14', bio: '평생 목공일. 아이들에게 나무로 뭐든 만드는 법을 가르쳐주고 싶어요. (서류 검토 중)' },
      { id: 'p106', name: '서정애', gender: 'F', type: 'senior', age: 76, phone: '010-1616-1717', address: '광주광역시 광산구 우산동 (35년 거주)', emergency_contact: '010-1818-1919 (딸)', occupation: '前 유치원 보육교사', skills: ['동화구연', '종이접기', '노래'], interests: ['손주', '합창', '화초'], needs: ['스마트폰 도움', '말벗'], availability: ['평일오전', '평일오후'], status: 'active', avatar_color: P.lavender, joined_at: '2027-05-25', bio: '유치원 보육교사 30년. 아이들에게 동화와 종이접기를 가르쳐주고 싶어요. (매칭 대기 중)' },
      { id: 'p204', name: '송미라', gender: 'F', type: 'parent', age: 36, phone: '010-2424-2525', address: '광주광역시 광산구 우산동', emergency_contact: '010-2626-2727 (배우자)', occupation: '물류회사 사무직', skills: [], interests: [], availability: ['평일 야근 잦음'], status: 'active', avatar_color: P.peach, joined_at: '2027-05-18', child_id: 'p304', bio: '야근이 잦아 아이 저녁 돌봄이 늘 숙제예요.' },
      { id: 'p205', name: '오정은', gender: 'F', type: 'parent', age: 33, phone: '010-2828-2929', address: '광주광역시 광산구 우산동', emergency_contact: '010-3131-3232 (모친)', occupation: '카페 운영', skills: [], interests: [], availability: ['주말 영업'], status: 'active', avatar_color: P.peach, joined_at: '2027-06-01', child_id: 'p305', bio: '주말에도 가게를 열어 아이와 시간을 못 보내 미안해요.' },
      { id: 'p206', name: '한소희', gender: 'F', type: 'parent', age: 37, phone: '010-3434-3535', address: '광주광역시 광산구 우산동', emergency_contact: '010-3636-3737 (배우자)', occupation: '병원 행정', skills: [], interests: [], availability: ['평일 하원 후'], status: 'active', avatar_color: P.peach, joined_at: '2027-06-12', child_id: 'p306', bio: '아이가 낯을 많이 가려서 따뜻한 어른들과의 만남이 필요해요. (매칭 대기 중)' },
      { id: 'p304', name: '송하준', gender: 'M', type: 'child', age: 8, phone: '', address: '광주광역시 광산구 우산동', emergency_contact: '010-2424-2525 (모친 송미라)', occupation: '초2', skills: [], interests: ['요리', '블록', '강아지'], availability: [], status: 'active', avatar_color: P.peach, joined_at: '2027-05-18', parent_id: 'p204', bio: '요리하는 걸 좋아하고 호기심이 많아요.' },
      { id: 'p305', name: '오서윤', gender: 'F', type: 'child', age: 7, phone: '', address: '광주광역시 광산구 우산동', emergency_contact: '010-2828-2929 (모친 오정은)', occupation: '초1', skills: [], interests: ['그림', '장기', '동물'], availability: [], status: 'active', avatar_color: P.peach, joined_at: '2027-06-01', parent_id: 'p205', bio: '그림 그리기와 보드게임을 좋아해요.' },
      { id: 'p306', name: '김라온', gender: 'M', type: 'child', age: 9, phone: '', address: '광주광역시 광산구 우산동', emergency_contact: '010-3434-3535 (모친 한소희)', occupation: '초3', skills: [], interests: ['공룡', '종이접기', '책'], availability: [], status: 'active', avatar_color: P.peach, joined_at: '2027-06-12', parent_id: 'p206', bio: '낯을 가리지만 종이접기엔 푹 빠져요. (매칭 대기 중)' },
    ],
    ...
    activity_logs: [
      ...
      { id: 'log108', activity_id: 'act108', participant_id: 'p002', hours: 1.5, summary: '도윤이 받아쓰기 — 80점에서 95점으로!', approved: false, approved_at: null, approved_by: null, has_photo: false, mood: 5, revision: { status: 'requested', note: '활동 내용이 한 줄이라 무엇을 했는지 확인이 어렵습니다. 진행한 순서와 아이 반응을 조금만 더 적어주세요.', requested_at: '2027-07-06', requested_by: '코디 한가은' } },
  ...
  return markDemo(data);
}

/** 시드에 담긴 테이블 이름 목록 (문서·초기화 버튼용) */
export function seedTableNames() {
  return Object.keys(buildSeed());
}

/** 시드 레코드 총 건수 — 「데모 데이터 N건」 표기용 (하드코딩 금지, 항상 계산) */
export function seedRecordCount(seed) {
  const s = seed || buildSeed();
  return Object.values(s).reduce((n, rows) => n + (Array.isArray(rows) ? rows.length : 0), 0);
}
